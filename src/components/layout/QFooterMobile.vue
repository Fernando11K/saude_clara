<template>
    ...
    <q-footer v-if="q.platform.is.mobile" reveal elevated class="bg-blue-8 glossy text-white ">
        <q-separator class="shadow-up-24 glossy bg-grey-1" />
        <q-toolbar class="bg-red-14 glossy">
            <div class="row justify-around col-12 fonte-footer ">
                ...
                <q-btn size="md" flat round class="fonte-footer ">
                    <q-icon size="md" name="fa-solid fa-pen fa-bounce">
                    </q-icon>
                </q-btn>
            </div>
        </q-toolbar>
...
const q = useQuasar()
const emits = defineEmits(['statusMenuLateral'])

const toggleLeftDrawer = () => { emits('statusMenuLateral') }
